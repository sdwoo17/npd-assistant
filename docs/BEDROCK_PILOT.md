...
별도 도메인 팩은 같은 명령에 `--assets /path/to/private-pack`을 추가한다. 자료가 동일하다면 기존 확보본을 재사용하며 ZIP을 공개 코드에 넣지 않는다. 원문과 초안 인사이트는 기본 비공개다. 소유자가 화면에서 인사이트를 검토·공개한 뒤 리서치 화면의 프로필 초안을 활성화한다. 기존 버전으로 적재한 폴더에는 `scripts/register_persona_templates.py`로 동일한 팩의 보류 프로필만 등록한다. 원래 적재 영수증과 전체 입력 해시를 확인하며 기존 데이터와 PO 편집은 유지한다. 새 데이터 폴더에 전체 인사이트를 검토해 공개하는 `--publish-insights`도 유지한다. [비공개 팩 형식과 범위](PRIVATE_DEMO_PACK.md)를 참고한다.
...
전체 흐름 검사는 별도 임시 DB와 합성 자료를 사용하고 종료 시 제거한다. 고객 DB를 수정하지 않는다. `--passes 3`은 실제 유료 모델 호출을 세 번의 전체 흐름으로 반복한다. HTTP 시험 서버도 로컬에서 별도로 실행한다. HTTPS 배포 주소에는 `python scripts/https_validation.py --url https://YOUR_DOMAIN --run --passes 3`을 사용한다. 별도 소유자·PO 계정을 숨김 입력하며, 기존 서비스 데이터와 분리한 합성 검증 프로젝트를 생성하고 검토용으로 남긴다. 실제 UI 과업은 추가 PO UAT로 확인한다. 출력은 단계·시각·건수만 포함하며 원문·비밀번호·모델 답변을 로그에 기록하지 않는다.
...
5. 검색 정답 집합과 사람이 판단한 기능 라벨로 품질을 평가한다. 2026-09-13 검색 수정 후 작성된 도메인 질문 8개 모두 상위 24개에 기대 후보가 포함됐지만, 독립 평가셋·상위 순위·실제 답변 의미 품질 검증을 대체하지 않는다. 합성 VoC의 라벨 적재 성공은 모델 분류 정확도가 아니다.
6. AXIOM에는 승인된 Markdown/JSON을 전달하고 수신 뒤 인용·버전이 보존되는지 확인한다. AXIOM API 직접 연결은 별도 범위다.

배포 템플릿은 암호화 EBS를 인스턴스 종료 후 보존하지만 자동 백업·재연결·복원을 구현한 것은 아니다. 기존 인스턴스 교체나 업그레이드 전에 `scripts/pilot_ops.py`로 DB와 로컬 암호화 키를 하나의 암호화 백업에 저장하고 새 폴더 복구를 검증한다. 예약 실행·외부 저장·인스턴스 재연결은 별도 운영 작업이다. [운영 절차](PILOT_OPERATIONS.md)를 따른다. Cognito, 관리형 DB, KMS 기반 문서별 암호화, HA, 작업 큐, 대규모 부하 검증은 이 파일럿에 포함되지 않는다. 기존 CloudFront 앱도 변경되지 않는다.
