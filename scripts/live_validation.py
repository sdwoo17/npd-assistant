    ...
    try:
        with tempfile.TemporaryDirectory(prefix='npd-live-') as tmp:
            ...
            try:
                ...
                assert drafts and marker not in json.dumps(drafts,ensure_ascii=False)
                assert not any(e['kind']=='insight' for e in call(po,'/api/evidence'))
                ...
                assert len([m for m in second['messages'] if m.get('is_synthetic')])==4
                ...
                assert applied['applied_prd_version']==2
                export=call(po,'/api/export/'+conv['id']+'?format=markdown')
                assert export['text'] and marker not in export['text']
                assert marker not in json.dumps(call(po,'/api/bootstrap'),ensure_ascii=False)
                assert '['+drafts[0]['id']+']' in export['text'] or any('['+e['id']+']' in export['text'] for e in call(po,'/api/evidence'))
                before=len(call(po,'/api/evidence'))
                call(owner,'/api/insights/release',{'insight_ids':[d['id'] for d in drafts],'published':False})
                assert len(call(po,'/api/evidence'))==before-len(drafts)
                assert not call(po,'/api/proposals')
